...
void leaveRoom(int creatureType) {
    for (int direction = 0; direction < 4; direction++) {
        ...
        if (direction == 0) { // Check east
            targetRoom = currentRoom->east;
            exitDirection = currentRoom->eastNum;
        } else if (direction == 1) { // Check west
            targetRoom = currentRoom->west;
            exitDirection = currentRoom->westNum;
        } else if (direction == 2) { // Check north
            targetRoom = currentRoom->north;
            exitDirection = currentRoom->northNum;
        } else { // Check south
            targetRoom = currentRoom->south;
            exitDirection = currentRoom->southNum;
        }

        if (targetRoom != NULL && exitDirection != -1) {
            for (int i = 0; i < currentRoom->creatureCounter; i++) {
                if (currentRoom->creatures[i].type == creatureType) {
                    // Find an empty spot in the target room
                    for (int j = 0; j < 10; j++) {
                        if (targetRoom->creatures[j].type == 0) {
                            targetRoom->creatures[j] = currentRoom->creatures[i];
                            targetRoom->creatures[j].location = exitDirection; // Update creature's location
                            targetRoom->creatures[j].type = targetRoom->state == 1 ? 2 : 1; // Set creature's type
                            targetRoom->creatureCounter++;
                            printf("%d is dissatisfied with the room and leaves to %d\n", currentRoom->creatures[i].creatureNum, exitDirection);
                            // Remove the creature from the current room
                            currentRoom->creatures[i].type = 0;
                            currentRoom->creatureCounter--;
                            return; // Exit the function after one creature has moved
                        }
                    }
                }
            }
    ...
    printf("No available exit for the creature type %d.\n", creatureType);
}
